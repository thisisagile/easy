...
export class Map<P extends Property = Property> {
  ...
  in = (from?: Json): Json => this.properties.reduce((a: any, [k, p]: [string, P]) => clone(a, k, p.name, p.options.def, p.options.convert.to), { ...from });
  out = (to?: Json): Json => this.properties.reduce((a: any, [k, p]: [string, P]) => clone(a, p.name, k, undefined, p.options.convert.from), { ...to });
}
