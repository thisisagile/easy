...
export class Exception extends Enum {
  static readonly CouldNotExecute = (target: Text, error: Text) => new Exception(`Could not execute on ${target} because ${error}.`);
  ...
  static readonly EnvironmentVariableNotFound = (variable: Text) => new Exception(`Environment variable ${text(variable).upper} could not be found.`);
