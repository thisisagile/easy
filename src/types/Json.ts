...
export const json = {
  ...
  merge: (...subjects: unknown[]) => subjects.map(s => json.parse(s)).reduce((js, j) => ({...js, ...j}), {}),
  omit: (subject: unknown, ...keys: string[]): Json => keys.reduce((js, k) => { delete js[k]; return js}, json.parse(subject)),
};
