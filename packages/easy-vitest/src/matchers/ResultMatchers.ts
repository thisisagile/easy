...
import { MatcherResult, Results } from '../utils/Types';
...
export const toResultWith = (results: Results, message: string): MatcherResult =>
  match(results)
...
export const toFail = (results: Results): MatcherResult =>
  match(results)
    .undefined(r => r, notDefined)
    .not(r => !r.isValid, doesNotFail)
    .else('Results does not fail');

export const toFailWith = (results: Results, message: string): MatcherResult =>
  match(results)
    .undefined(r => r, notDefined)
    .not(r => !r.isValid, doesNotFail)
    .not(
